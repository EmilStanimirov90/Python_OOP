...
class Television(RCAMixin, HDMIMixin, PowerOutlet, Device):
...
class DVDPlayer(HDMIMixin, PowerOutlet, Device):
...
class GameConsole(HDMIMixin, EthernetMixin, PowerOutlet, Device):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
...
class Router(EthernetMixin, PowerOutlet, Device):
    def connect_to_tv(self, television):
