...
class TestList(TestCase):
    def setUp(self):
        self.integer_list = IntegerList('5', 1, False, 3.5, 2, 3)

    def test_correct_init_and_get_date(self):
    ...
    def test_add_operation_with_float_raises_error(self):
        with self.assertRaises(ValueError) as ve:
            self.integer_list.add(5.5)
        self.assertEqual("Element is not Integer", str(ve.exception))

    def test_remove_index_operation_without_index_raises_error(self):
    ...
    def test_get_with_invalid_index_returns_element(self):
        expected_element = 2
    ...
    def test_insert_element_with_invalid_index_raise_error(self):
        with self.assertRaises(IndexError) as ie:
