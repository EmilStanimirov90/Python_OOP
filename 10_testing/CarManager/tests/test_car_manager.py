...
class TestCarManager(TestCase):
    ...
    def test_no_model_raises_exception(self):
        with self.assertRaises(Exception) as ei:
            model = Car("Nissan", "", 15, 75)
        self.assertEqual("Model cannot be null or empty!", str(ei.exception))

    def test_zero_fuel_consumption_raises_exception(self):
        with self.assertRaises(Exception) as ei:
            fuel_consumption = Car("Nissan", "GT-R", 0, 75)
        self.assertEqual("Fuel consumption cannot be zero or negative!", str(ei.exception))

    def test_zero_fuel_capacity_raises_exception(self):
        with self.assertRaises(Exception) as ei:
            fuel_consumption = Car("Nissan", "GT-R", 15, 0)
        self.assertEqual("Fuel capacity cannot be zero or negative!", str(ei.exception))
